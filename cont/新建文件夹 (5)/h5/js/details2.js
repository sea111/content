			...
			autoPlay:function(){
				...
				if(productInfo.productDetails.playUrl&&version>=35.2){
					...
					$(".videoWrap div").on('click',function(){
						WPBridge.callMethod("JsInvokeNative","wpHitDotEvent",{eventId:"h5_e028"},'');	
						WPBridge.callMethod("JsInvokeNative","wpNetwork",{},function(msg){
							if(msg.data.result=='2'){
								...
								WPBridge.callMethod("JsInvokeNative","wpShowToast",{message:"无连接，请检查网络"},'');	
							}	
						});
		...
		$(function(){
          ...
          //根据起点和终点返回方向 1：向上，2：向下，3：向左，4：向右,0：未滑动  
         	...
         	$("#content").get(0).addEventListener('touchstart',function (ev) {  
         		if(ev.target.className=="goods_pic"||ev.target.className=='videoWrap'||ev.target.tagName=='VIDEO'||ev.target.className=="swiper-slide swiper-slide-next"){
         			return false;
         		}
...
//							return;
		...
		function getData(){
			...
			$(".share").on("touchstart",function(){
				...
				WPBridge.callMethod('JsInvokeNative','wpShareProduct',{productId:prtId,shareTitle:title,shareText:text,shareUrl:"https://share.wpwl.org/h5/share.html?productId="+prtId,shareImg:imgUrl},function(){});
			})
			//附近门店
