...
function getKeyStr(){
    $.ajax({
        ...
        success: function (res) {
            ...
            pointDatas[0] = pointDataList;
            console.log(pointDatas);
            wpwlDetails.pointPush(pointDatas);
        }
    });
}
...
function verrifyFake(baseKeyStr){
    ...
    pointDataList.acPointLog.pointCode="e000";
	...
	antifakeDataList.acAntifakeLog.verifyResult = "1";	//验伪结果  
   	antifakeDataList.sessionId = sessionID;
    //点击进入进行验伪打点
    
    antifakeDatas[0] = antifakeDataList;
    // console.log(antifakeDatas);
