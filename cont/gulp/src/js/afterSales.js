$(function(){
	...
	var listData;//接口中数据
	var cId=getUrlRequest().suspectId;//cId是用户id
	var changeCont=0;//判断显示的是售后申请的内容还是售后进度的内容
	var length;
	//售后申请和售后进度切换
	$(".after-application").click(function(){
		changeCont=0;//显示的是售后申请的内容
	...
	$(".aftermarket-schedule").click(function(){
		...
		if(!this.flag){//this.flag默认是undefined
			getData('listprogression');
			this.flag=true;
		}
		wpCommon.viewShow();//点击两次售后进度时显示loading且不消失,调用关闭loading
	})
	//加密
