$(function(){
	...
	getKey();	
	var scrollEndTimer=null;//清除倒计时
	var pageIndex={//判断显示的是失效还是不失效优惠券
		disCountContent:1,
		undisCountContent:1
	}
	var pageSize=6;//显示6张数据
	var complete=true;//加载完成
	//获取数据
	function getData(type,key){
		...
		if(complete){
			complete=false;
			var suspectId=urlRequest['suspectId'];//用户id
			var pageIn=pageIndex[type];//页数
	...
	$("#discountContent").on("click",".discountBox",function(){
		...
		var coupId=$(this).attr("couponId");//优惠券id
		var activityId=$(this).attr("activityId");//活动id
		var couponName=$(this).attr("couponName").slice(0,-3);//优惠券名字
